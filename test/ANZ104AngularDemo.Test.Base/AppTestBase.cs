...
using ANZ104AngularDemo.Test.Base.TestData;
...
namespace ANZ104AngularDemo.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="ANZ104AngularDemoDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(ANZ104AngularDemoDbContext context)
            {
        ...
        protected void UsingDbContext(Action<ANZ104AngularDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ANZ104AngularDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<ANZ104AngularDemoDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<ANZ104AngularDemoDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ANZ104AngularDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ANZ104AngularDemoDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ANZ104AngularDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ANZ104AngularDemoDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<ANZ104AngularDemoDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<ANZ104AngularDemoDbContext, Task<TResult>> func)
        {
            TResult result;
